...
function approximateTokenCountByChars($text) {
    ...
    return ceil($charCount / 3); // Rough approximation: 4 characters per token
}
...
function call_azure_api($active_config, $msg) {
    ...
    if (!empty($active_config['max_tokens'])) $payload['max_tokens'] = $active_config['max_tokens'];
...
function get_chat_thread($message, $chat_id, $user, $active_config)
{
    ...
    // Add the past exchanges from the recent chat history to the messages array
    $recent_messages = get_recent_messages($chat_id, $user);
    
    $tokenLimit = $context_limit ; // Set your token limit here
    
    // estimate total tokens
    $total_tokens = approximateTokenCountByChars($message);
    ...
    if (!empty($recent_messages)) {
        ...
        // Build the prompt context from the most recent to oldest
        foreach (array_reverse($recent_messages) as $message) {
            // Retrieve stored token lengths
            $prompt_tokens = $message['prompt_token_length'];
            $reply_tokens = $message['reply_token_length'];

            // Tokens needed for this exchange (include message formatting tokens)
            $tokens_needed = $prompt_tokens + $reply_tokens + (4 * 2); // 4 tokens per message as per OpenAI's tokenization

            // Check if adding this exchange exceeds the context limit
            if ($total_tokens + $tokens_needed > $context_limit) {
                // Exceeds limit, stop adding more exchanges
                break;
            }

            // Include assistant's reply
            $formatted_messages[] = ['role' => 'assistant', 'content' => $message['reply']];
            // Include user's prompt
            $formatted_messages[] = ['role' => 'user', 'content' => $message['prompt']];
